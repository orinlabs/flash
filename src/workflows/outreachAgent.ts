...
function buildSeedUserMessage(input: {
  ...
  const mailboxBlock = company.mailbox
    ? [
        ...
        'Do NOT include any signature or sign-off footer in draft_email body — it is appended automatically on send.',
        company.mailbox.signature
          ? `Mailbox signature (appended on send only; never copy into draft body):\n${company.mailbox.signature}`
          : '(No mailbox signature configured yet — still do not add a signature block in the draft body.)'
      ]
        .filter(Boolean)
        .join('\n')
